...
(defn my-but-last [& lst]
  (if (<= (count lst) 2) lst
      ;; reverse lst if lst is too large
      (let [rev (reverse lst)]
...
(defn element-at [lst pos]
  (if (or (>= pos (count lst)) (< pos 0)) nil
      (nth lst pos)))

;; 4
;; Find the number of elements of a list.
(defn find-element [lst e]
  (first (filter (fn [x] (= x e)) lst)))
...
(defn my-reverse
  ...
  ([lst coll]
     (if (empty? lst) coll
         (recur (rest lst) (cons (first lst) coll)))))
(defn my-reverse2 [lst]
  (loop [[f & r :as c] lst coll '()]
    (if (empty? c) c
        (recur r (cons f coll)))))
...
(defn my-palindrome [lst]
  (loop [f 0 l (dec (count lst))]
    (if (or (= f l) (> f l)) true
...
(defn my-flatten [lst]
  (when coll?
    (cond
     (empty? lst) lst
     (coll? (first lst)) (lazy-cat (my-flatten (first lst)) (my-flatten (rest lst)))
     :else (lazy-cat
...
;; how to think recusivly(one step and one part step)
(defn pack-list [lst]
  (if (= lst nil)
    nil
...
(defn pack [lst]
  (loop [xs lst res '()]
    (cond
     (empty? xs) res
     :else (recur (drop-while (fn [x] (= x (first xs))) xs)
                  (cons (take-while (fn [x] (= x (first xs))) xs) res)))))
...
(defn encode [lst]
  (loop [xs lst res '()]
    (cond
     (empty? xs) res
     :else (recur (drop-while (fn [x] (= x (first xs))) xs)
                  (cons (let [tmp (take-while (fn [x] (= x (first xs))) xs)]
                          (list (count tmp) (first tmp))) res)))))
...
(defn encode-modified [lst]
  ((fn [xs]
     (loop [[f & r] xs res '()]
       (if (empty? r) res
           (recur r (cons (if (= 1 (first f)) (nth f 1)
                     f) res))))) (encode lst)))
;; P12 (**) Decode a run-length encoded list.
;; Given a run-length code list generated as specified in problem P11. Construct its uncompressed version.
(defn decode [lst]
  (loop [[f & r]lst res '()]
       (if (and (nil? f) (empty? r)) res
           (recur r (cons (if (coll? f) (repeat (first f) (last f))
                              f) res)))))
...
;; * (repli '(a b c) 3)
;; (A A A B B B C C C)
(defn repli [lst times]
  (mapcat (fn [x] (repeat times x)) lst))
...
(defn drop-nth [lst n]
  (let [dlst
        (loop [xs lst res '()]
          (if (empty? xs) res
              (recur (drop n xs) (cons (take (dec n) xs) res))))]
    (my-flatten dlst)))
...
(defn rotate [lst n]
  (if (> n 0) (take (count lst) (drop n (cycle lst)))
      (take (count lst) (drop (- (count lst) (Math/abs n)) (cycle lst)))))

(defn rotate-1 [lst n]
  (if (> n 0) (my-flatten (list (drop n lst) (take n lst)))
      (my-flatten (list (drop (- (count lst) (Math/abs n)) lst) (take (- (count lst) (Math/abs n)) lst)))))
...
;; (A ALFA B C D)
(defn insert-at [a lst n]
  (concat (take (dec n) lst) (list a) (drop (dec n) lst)))
...
(defn lotto-select [x y]
  (let [lst (range (inc y))]
    (loop [set #{}]
      (if (= x (count set)) (seq set)
          (recur (conj set (nth lst (rand y))))))))
...
(defn rnd-permu
  ([lst] (rnd-permu lst (count lst)))
  ([lst n]
     (when (> n 0)
       (let [p (rand-int (dec n))]
         (lazy-cat (list (nth lst p)) (rnd-permu (remove-at lst (inc p)) (dec n)))))))
...
;; ((A B C) (A B D) (A B E) ... )
(defn perm-helper [e lst n]
  (cond
   (> n 1) (for [x lst ]
             (perm-helper (my-flatten (list e x)) (filter #(not (= x %)) lst) (dec n)))
   :else (for [x lst]
           (my-flatten (list e x)))))
(defn permutation [n lst]
  (if (> n (count lst)) nil
      (for [x lst]
        (perm-helper x (filter #(not (= x %)) lst) (dec n)))))
...
;; Arithmetic
(defn lfsort [lst]
  (let [m (map (fn [x] (count x)) lst)]
    (map last (sort-by first (map (fn [x] (list (count (filter #(= % (count x)) m)) x)) lst)))))
...
(defn is-prime [n]
  (cond
   (= n 1) false
   (= n 2) true
   :else (every? (fn [x] (not (zero? (rem n x)))) (range 2 (inc (Math/sqrt n))))))
...
;; 4
(defn totient-phi [n]
  (count (for [x (range n) :when (coprime n x)] x)))

(defn phi [m]
  (count (filter (partial coprime m) (range m))))
...
(defn prime-factors [n]
  (loop [m 2 x n res []]
    (cond
     (= 1 x) (seq res)
     (zero? (rem x m)) (recur 2 (quot x m) (conj res m))
     :else (recur (inc m) x res))))
...
(defn prime-factors-mult [n]
  (let [f (prime-factors n)]
    (map (fn [x] (list x (count (filter #(= % x) f)))) (seq (set f)))))
...
(defn phi-improved [m]
  (let [calculate (fn [x] (let [p1 (first x) m1 (last x)] (* (dec p1) (Math/pow p1 (dec m1)))))]
    (reduce + (map calculate  (prime-factors-mult m)))))
...
(defn primes [lower upper]
  (for [x (range lower upper) :when (is-prime x)] x))
...
(defn goldbach [n]
  (let [prs (primes 1 n)]
    (loop [[f & l] prs]
      (when (last l)
        (cond
         (= (+ f (last l)) n) (list f (last l))
         (< (+ f (last l)) n) (recur l)
         (> (+ f (last l)) n) (recur (drop-last l)))))))
...
(defn goldbach-list [lower upper]
  (filter #(even? (+ (first %) (last %)))
          (for [x (range lower (inc upper)) :let [res (goldbach x)] :when (not (nil? res))] res)))
